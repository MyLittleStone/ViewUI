    ...
    <thead>
      <tr v-for="(cols, rowIndex) in headRows">
        <th
          ...
          :key="index"
          @mousedown="handleTitleMouseDown(column, index, $event)"
        >
          <div :class="cellClasses(column)">
            ...
            <template v-else>
              <span
                ...
                :class="{
                  [prefixCls + '-cell-sort']: column.sortable,
                  moving: move,
                }"
                @click="
                  column.sortable &&
                    handleSortByHead(getColumn(rowIndex, index)._index)
                "
                >{{ column.title || "#" }}</span
              >
              <render-header
                ...
                >
                  <ul :class="[prefixCls + '-filter-list-single']">
                    ...
                    <li
                      ...
                      @click="
                        handleSelect(
                          getColumn(rowIndex, index)._index,
                          item.value
                        )
                      "
                    >
...
export default {
    ...
    isSelectAll() {
      ...
      for (let i in this.objData) {
        ...
        if (!(objData._isDisabled && !objData._isChecked)) {
          ...
          isAllDisabledAndUnSelected = this.isChildrenAllDisabledAndUnSelected(
            objData,
            isAllDisabledAndUnSelected
          );
        }
      }
      ...
      if (this.data.length) {
        for (let i in this.objData) {
          ...
          if (!objData._isDisabled) {
            ...
            isSelectDisabled = this.isChildrenDisabled(
              objData,
              isSelectDisabled
            );
          }
  ...
  methods: {
    ...
    handleMouseDown(column, event) {
      ...
      if (this.draggingColumn) {
        ...
        const columnEl = this.$el.querySelector(
          `th.ivu-table-column-${column.__id}`
        );
    ...
    handleTitleMouseDown(column, index, event) {
      if (this.$isServer || !this.move || this.moving) return;
      console.log("mouse down");
      column._moveState = "moving";
      ...
      const handleMouseMove = (e) => {
        ...
        // console.log(`mouse move 1 ${e.clientX}`);
        // 找到合适位置，
        let childrenEls = this.$el.querySelectorAll("th");
        let useIndex = 0;
        let insertWidth = 0;
        for (; useIndex < childrenEls.length; useIndex++) {
          let th = childrenEls[useIndex];
          let thRect = th.getBoundingClientRect();
          if (e.clientX >= thRect.left && e.clientX < thRect.right) {
            if (useIndex > this.movingIndex) {
              insertWidth = thRect.right - (tableRect.left + 1);
            } else if (useIndex < this.movingIndex) {
              insertWidth = thRect.left - tableRect.left + 1;
            }
            break;
          }
        }
        this.movingTo = useIndex;
        if (useIndex == this.movingIndex) {
          table.showResizeLine = false;
          return;
        }
        if (table.border) {
          if (useIndex + 1 == childrenEls.length) insertWidth -= 3;
          else insertWidth -= 2;
        }
        resizeProxy.style.left = insertWidth + "px";
        resizeProxy.classList.add("moving");
        table.showResizeLine = true;
      };
      const handleMouseUp = () => {
        // console.log("mouse up 1");
        column._moveState = "move";
        this.moving = false;
        ...
        document.ondragstart = null;
        table.showResizeLine = false;
        resizeProxy.classList.remove("moving");
        table.$emit(
          "on-column-position-changed",
          this.movingIndex,
          this.movingTo,
          column,
          event
    ...
    // handleTitleMouseMove(column, event) {
